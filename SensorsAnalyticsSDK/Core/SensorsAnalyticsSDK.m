...
// [自定义修改] 自定义请求头 setter/getter 实现
#pragma mark - Custom Headers

- (void)setCustomHeaders:(nullable NSDictionary<NSString *, NSString *> *)customHeaders {
    dispatch_async(self.serialQueue, ^{
        if (customHeaders) {
            self.configOptions.customHeaders = [customHeaders copy];
        } else {
            self.configOptions.customHeaders = nil;
        }
    });
}
...
- (nullable NSDictionary<NSString *, NSString *> *)customHeaders {
    ...
    dispatch_sync(self.serialQueue, ^{
        headers = self.configOptions.customHeaders;
    });
    return headers;
}
